    ...
    return (
      <div className="page-wrapper">
        ...
        <div className="container" style={{ paddingTop: 60, textAlign: "center" }}>
          ...
          <button className="btn btn-primary mt-16" onClick={() => navigate("/home")}>
            Back to dashboard
          </button>
        </div>
      </div>
      ...
      <div className="container result-container">
        ...
        <div className="result-header">
          ...
          <div className="result-header-actions">
            <button className="btn btn-ghost" onClick={() => navigate("/interview/history")}>
        ...
        <div className="result-layout">
          ...
          <div className="result-main">
            ...
            <div className="card result-overview">
              <div className="overview-score">
                <div className="big-score">{record.overallScore ?? "—"}<span>/10</span></div>
                <div className="score-caption">{content.sections.overallScore}</div>
              </div>
              ...
              <div className="overview-readiness">
                ...
                <div className="score-caption">{content.sections.readiness}</div>
              </div>
              <div className="overview-meta">
                <span className="meta-item">🎯 {record.targetRole}</span>
                {record.targetCompany && <span className="meta-item">🏢 {record.targetCompany}</span>}
            ...
            <div className="card mt-20">
              <h2 className="section-heading mb-20">{content.sections.breakdown}</h2>
            ...
            {record.rounds?.length > 0 && (
              <div className="card mt-20">
                <h2 className="section-heading mb-20">{content.sections.roundWise}</h2>
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {record.rounds.map((round, ri) => (
                    <div key={ri} className="round-block">
                      ...
                      >
                        <div className="flex items-center gap-12">
                          <span className="badge badge-primary">
                            {content.roundLabels[round.roundType] || round.roundType}
                          </span>
                      ...
                      {expandedRound === ri && round.turns?.map((turn, ti) => (
                        <div key={ti} className="turn-block">
                          <div className="turn-section">
                            <span className="turn-label">{content.turnLabels.question}</span>
                            <p className="turn-text">{turn.question}</p>
                          </div>
                          <div className="turn-section">
                            <span className="turn-label">{content.turnLabels.yourAnswer}</span>
                            <p className="turn-text">{turn.answer}</p>
                          </div>
                          {turn.improvedAnswer && (
                            <div className="turn-section improved">
                              <span className="turn-label">{content.turnLabels.betterAnswer}</span>
                              <p className="turn-text">{turn.improvedAnswer}</p>
                            </div>
                          )}
                          {turn.scores && (
          ...
          <div className="result-sidebar">
            ...
            {record.finalSummary && (
              <div className="card">
                <h3 className="section-heading mb-12" style={{ fontSize: "1rem" }}>
                  {content.sections.summary}
                </h3>
                <p className="body-text" style={{ fontSize: "0.9rem" }}>{record.finalSummary}</p>
              </div>
            )}
            ...
            {record.strengths?.length > 0 && (
              <div className="card mt-16">
                ...
                <ul className="feedback-list strengths-list">
                  {record.strengths.map((s, i) => <li key={i}>{s}</li>)}
                </ul>
              </div>
            )}
            ...
            {record.weaknesses?.length > 0 && (
              <div className="card mt-16">
                ...
                <ul className="feedback-list weaknesses-list">
                  {record.weaknesses.map((w, i) => <li key={i}>{w}</li>)}
                </ul>
              </div>
            )}
