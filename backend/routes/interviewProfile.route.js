...
router.get("/", protect, async (req, res) => {
  try {
    const profiles = await InterviewProfile.find({ userId: req.user._id }).sort(
      { createdAt: -1 },
    );
    res.json(profiles);
  } catch (error) {
...
router.post("/", protect, async (req, res) => {
  try {
    ...
    if (!label || !reason || !targetRole) {
      return res
        .status(400)
        .json({ message: "Label, reason, and target role are required" });
    }
    if (isDefault) {
      await InterviewProfile.updateMany(
        { userId: req.user._id },
        { isDefault: false },
      );
    }
